...
//obligitory change so i can commit
...
function prompt8() {
  someNumber = parseInt(100*Math.random());
  console.log(someNumber);
 if (someNumber < 30){console.log('That\'s a small number!');}
  if (someNumber >= 30 && someNumber <= 60){console.log('That number is medium sized.');}
   if (someNumber > 60){console.log('We got a big one!');}
}
...
function prompt9() {
 for (let i=0;i<starWars.length; i++){console.log(i + ". " + starWars[i])}
...
function prompt11() {
  ...
  //half the number of items used to find middle
  let items = nums.length/2;
  let med1;
  let med2;
    if (2*items%2 === 0){
    med1 = nums[Math.floor(items)];
    med2 = nums[Math.floor(items)+1];
    median = (med1+med2)/2;
  }
   else {median = nums[Math.floor(items)+1];
    }
...
function prompt13() {
  for (let i = 0; i<nestedArrays.length;i++){
    for (let j = 0; j<nestedArrays[i].length; j++){console.log(nestedArrays[i][j])}
  }
}
