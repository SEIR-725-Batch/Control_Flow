...
function prompt7() {
  for (let i = 0; i <= 100; i++) {
    ...
    paragraph.innerText = "I found a " + i + ". High five!"
...
var someNumber = 100 * (Math.random())
...
function prompt9() {
  for(let i = 0; i < 6; i++) {
    let index = starWars.indexOf(starWars[i])
    console.log(index + ". " + starWars[i])
  }
}
 ...
 * 
 * run (nums.sort), array length is 25, so the median may be a decimal. add in the math calc to find the median. average of index 12 and 13. use math floor because it seems they want an integer
 */
