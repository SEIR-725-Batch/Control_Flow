...
function prompt6() {
  for (let i = 0; i<=50; i+=2) {console.log(i)}
}
...
// COMMENT IN THE BELOW LINE OF CODE TO TEST YOUR OUTPUT IN THE BROWSER!
 //prompt8();
// I'm so confused by how to solve this
...
function prompt9() {
  //let starWars = ["0. Han", "1. C3P0", "2. R2D2", "3. Luke", "4. Leia", "5. Anakin"];
  starWars.forEach (console.log(starWars))
}
...
//Got this solution from educative.io to see if it would work/I could understand it. This is not my own work.
// COMMENT IN THE BELOW LINE OF CODE TO TEST YOUR OUTPUT IN THE BROWSER!
//prompt11();
...
function prompt12() {
  for (let i = 1; i <= 10; i++) {
    for let (j = 11; j <=20; j++) {console.log('i: ${i} / j: $j{j}')}
  }
}
...
function prompt13() {
  if (typeof(arr) == "object") {
    for (var i = 0; i < arr.length; i++) {printArray(arr[i])} 
    {for (var j = 0; j < arr.length; j++) {printArray(arr[j])}} 
    {for (var k = 0; k < arr.length; k++) {printArray(arr[k])}}
  }
}
