...
var someNumber = Math.random()*100;
// console.log(someNumber);

function prompt8() {
      if (someNumber <30) {
        console.log("Now that's what I call a small number!");
      }
      else if (someNumber >30 && someNumber < 60) {
        console.log("Okay pal, what yer dealin' with here is a medium-sized number.");
      }
      else if (someNumber >60) {
        console.log("We got a big'n here, fellers!");
    }
  }
// COMMENT IN THE BELOW LINE OF CODE TO TEST YOUR OUTPUT IN THE BROWSER!
// prompt8();
...
function prompt12() {for (let i = 1; i <= 10; i++ )
  {
   for (let j = 11; j <= 20; j++)
     console.log(`i: ${i} / j: ${j}`)};
}
...
function prompt13() {
  ...
  for ( i = 0; i < nestedArrays.length; i++) {
    for (j=0; j < nestedArrays[i].length; j++)
        results = nestedArrays[i].toString();
      console.log(results);
}
}
