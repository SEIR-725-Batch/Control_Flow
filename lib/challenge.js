 ...
 * console.log that number. The median number is the middle number in a 
 * SORTED list of numbers. Think through the steps you'll need to take to 
 * determine this number with code!
 *
...
function prompt13() {
  ...
  for (let i = 0; i < arr.length; i++) {
    const nestedArrays = arr[i];
    for (let j = 0; j < nestedArrays.length; j++) {
      product *= nestedArrays[j];
    }
  }}
  return product

nestedArrays([[1,2,3],[4,5,6],[7,8,9]]);
}
