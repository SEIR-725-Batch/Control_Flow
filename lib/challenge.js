...
let someNumber = Math.floor(Math.random() * 99) + 1;
...
//COME BACK AND REDO THIS ONE LATER@@@@@@@@@@@@@@@@@@@@
//array.sort this and should be good
function prompt11() {
nums.sort()
let i = (nums.length/2) + 0.5;
let median = nums[i];
  console.log(median);
}
// COMMENT IN THE BELOW LINE OF CODE TO TEST YOUR OUTPUT IN THE BROWSER!
...
// //print template literal in the instructions means console.log it
...
function prompt13() {
  for (i = 0; i < nestedArrays.length; i++) {
    for (j = 0; j < nestedArrays[i]; j++) {
      console.log(nestedArrays[i][j]);
    }
  }
}
//not sure why this is not popping up in console.log
//have been stuck on this one for awhile; thought I had it because the code looks good to me
// COMMENT IN THE BELOW LINE OF CODE TO TEST YOUR OUTPUT IN THE BROWSER!
prompt13();
